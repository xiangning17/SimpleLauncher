...
//    public int leapMonth = 0; // 闰的是哪个月
    ...
    final static String[] Animals = new String[]{"鼠", "牛", "虎", "兔", "龙", "蛇",
            "马", "羊", "猴", "鸡", "狗", "猪"};
     ...
     * 从农历日期转换成公历日期
     * */
    public static Calendar getSunDate(int lunYear, int lunMonth, int lunDay, boolean isLeap) {
...
//        return "农历 "+lunM+lunD+" "+Festival;
     ...
     * @return
     */
    public String getLunarDate(int year, int month, int day, boolean isday) {
        int var = getLunarDateINT(year, month, day);
        // 农历年份
        int ly = (int) var / 10000;

        //农历月份
        int lm = (int) (var % 10000) / 100;
        int ld = var - ly * 10000 - lm * 100;
        if (!isday) {
        ...
        if (ld == 1)
            return chineseNumber[(lm - 1) % 12] + "月";
        else
            return getChinaDayString(ld);
    ...
    public static int getDateOfSolarTerms(int year, int month) {
     ...
     * dt：农历日
     * */
    public static String CalConv2(int yy, int mm, int dd, int y, int d, int m, int dt) {
        String dy = d + "" + dd;
    ...
    public static int getDaysOfMonth(int year, int month) {//返回公历year年month月的当月总天数
        if (month == 2) {
     ...
     * 公历某日宜忌
     * */
    public static String getyiji(Calendar calendar) {
        int year = calendar.get(Calendar.YEAR);
        int month = calendar.get(Calendar.MONTH) + 1;
        int day = calendar.get(Calendar.DATE);
